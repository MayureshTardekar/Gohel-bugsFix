import { useState } from "react";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Label } from "@/components/ui/label";
import { registerUser } from "@/lib/api";
import { useUser } from "@/context/UserContext";
import { toast } from "sonner";
import { Wallet, Twitter, Loader2 } from "lucide-react";

export default function AuthModal({ open, onClose, initialRef }) {
  const { signIn } = useUser();
  const [wallet, setWallet] = useState("");
  const [twitter, setTwitter] = useState("");
  const [loading, setLoading] = useState(false);

  const submit = async (e) => {
    e.preventDefault();
    if (!/^0x[a-fA-F0-9]{40}$/.test(wallet.trim())) {
      toast.error("Invalid EVM wallet address (must be 0x + 40 hex chars)");
      return;
    }
    if (!twitter.trim()) {
      toast.error("Twitter username required");
      return;
    }
    setLoading(true);
    try {
      const data = await registerUser({
        wallet_address: wallet.trim(),
        twitter_username: twitter.trim(),
        referred_by: initialRef || null,
      });
      signIn(data.user);
      toast.success(data.message || "Linked");
      onClose();
    } catch (err) {
      toast.error(err.response?.data?.detail || "Failed");
    } finally {
      setLoading(false);
    }
  };

  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent
        data-testid="auth-modal"
        className="bg-[#0d1017] border border-[#1b2234] text-slate-100 sm:max-w-md p-0 clip-terminal"
      >
        <div className="relative overflow-hidden scanning">
          <div className="p-8">
            <DialogHeader>
              <div className="font-mono-cyber text-[11px] uppercase tracking-[0.35em] text-[#00f0ff] mb-2">
                // Identity Link
              </div>
              <DialogTitle className="font-heading text-2xl font-bold uppercase tracking-tight">
                Bind your bone.
              </DialogTitle>
              <p className="text-sm text-slate-400 mt-2">
                Enter your wallet and Twitter. This is your on-chain skeletal signature.
              </p>
            </DialogHeader>
            <form onSubmit={submit} className="mt-8 space-y-5">
              <div>
                <Label className="font-mono-cyber text-[10px] uppercase tracking-[0.3em] text-[#00f0ff]">
                  Wallet Address
                </Label>
                <div className="relative mt-2">
                  <Wallet className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
                  <Input
                    data-testid="auth-wallet-input"
                    value={wallet}
                    onChange={(e) => setWallet(e.target.value)}
                    placeholder="0x…"
                    className="pl-10 bg-[#07080c] border-[#1b2234] rounded-none h-12 font-mono-cyber text-sm"
                  />
                </div>
              </div>
              <div>
                <Label className="font-mono-cyber text-[10px] uppercase tracking-[0.3em] text-[#00f0ff]">
                  Twitter Username
                </Label>
                <div className="relative mt-2">
                  <Twitter className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
                  <Input
                    data-testid="auth-twitter-input"
                    value={twitter}
                    onChange={(e) => setTwitter(e.target.value)}
                    placeholder="handle"
                    className="pl-10 bg-[#07080c] border-[#1b2234] rounded-none h-12 font-mono-cyber text-sm"
                  />
                </div>
              </div>
              {initialRef && (
                <div className="px-3 py-2 bg-[#00ff66]/10 border border-[#00ff66]/30 font-mono-cyber text-[11px] uppercase tracking-widest text-[#00ff66]">
                  Referred by: {initialRef}
                </div>
              )}
              <Button
                type="submit"
                disabled={loading}
                data-testid="auth-submit-btn"
                className="w-full rounded-none clip-notch h-12 bg-[#00f0ff] hover:bg-[#00f0ff]/90 text-black font-mono-cyber uppercase tracking-widest text-xs"
              >
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Link & Enter"}
              </Button>
            </form>
          </div>
        </div>
      </DialogContent>
    </Dialog>
  );
}
